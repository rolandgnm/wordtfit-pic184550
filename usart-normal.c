...
void main(void)
{
    ...
    int n = VETOR_AUXILIAR - 1; //MAX é o número de pontos da entrada
	float xbuscado; // Valor a ser buscado
	float xi[VETOR_AUXILIAR], yi[VETOR_AUXILIAR]; //Vetor auxiliar

	//vari�veis para definir vetor auxiliar
	int valorBuscado;
	int k, inicial;

	//vari�veis para calcular lagrange
	double result = 0.0;
    double polinomio;
	int i, j;

	//vari�veis auxilares para mostrar o float no LCD
	char resultado[10];
	int decimal, parte;

	//vari�veis para ler e escrever na serial
	char xbusc[TAM_NUM];
	char msg_1[11] = "\nEntrada: ";
	...
	printf("1234567890qwertyuiop1234567890qwertyuiop1234567890qwertyuiop1234567890qwertyuiop1234567890qwertyuiop");
	while(!DataRdyUSART());
	getsUSART(xbusc, TAM_NUM);
	putsUSART(xbusc);

	//Convers�o do x buscado para um valor float
	xbuscado = atof(xbusc);

    /*Define o vetor auxiliar*/
	valorBuscado = (int)xbuscado;
	if (valorBuscado <= 1) inicial = 0; //Se menor igual a 1, o vetor auxiliar é formado pelos 6 primeiros pontos do vetor original.
	else if  (valorBuscado > VETOR_PRINCIPAL-4) inicial = VETOR_PRINCIPAL-6; //Se maior que MAX-4, o vetor auxiliar é formado pelos 6 últimos pontos do vetor original.
	else inicial = valorBuscado-2; // Se não, o valor inicial são duas posições antes do buscado.
    ...
    //Chamada da função lagrange
	for (i=0;i<=n;i++)
	{
		polinomio=1.0;

		for (j=0;j<=n;j++)//Varia até o valor do grau do polinômio.
		if (i!=j)//i deve ser diferente de j para que o denominador não seja igual a zero.
		polinomio = polinomio * (xbuscado - xi[j]) / (xi[i] - xi[j]);//Polinômio interpolador de Lagrange.

		result = result + polinomio * yi[i];
	}

	//Convers�o do resultado float para string
	parte = result;
